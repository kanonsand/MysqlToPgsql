 ...
 */
public class HavingClauseHandler implements ClauseHandler {
    ...
    public void process(Object clause, ClauseContext ctx) {
        ...
        ((SQLExpr) clause).accept(new HavingVisitor(ctx));
    }
    
    private static class HavingVisitor extends MySqlASTVisitorAdapter {
        private final ClauseContext ctx;
        
        HavingVisitor(ClauseContext ctx) {
            this.ctx = ctx;
        }
